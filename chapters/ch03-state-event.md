...
flowchart TB
    ...
    subgraph main[" "]
        direction LR
        RT["Runtime (§2.4)"]
        ES["EventStore (append-only log)"]
        SV["SessionView (只读视图)"]
        RT -->|append| ES
        ES -->|fold| SV
    end

    SN["Snapshot (offset, V)"]
...
- **Event** — 唯一的**不可变**事实载体。所有状态变化 = 追加一条 Event。见 §3.3。
...
>
> **C2 · 确定性 Fold**:`fold(events)` 是纯函数——同一份事件流,任何时候折叠得到同样的 View。
...
| `payload`    | 类型化 | 见 §3.3.2                                                                 |
| `ts`         | 时间戳 | 由 EventStore 在 `Append` 时打;**仅作日志与跨系统跟踪,不作排序键,也不要求单调** |
...
type EventDTO struct {
    ...
    TS        string          `json:"ts"`          // RFC3339,与 Rust wire 一致
...
| 变更              | 例子                                             | 策略                                                                                                                                    |
| ----------------- | ------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------- |
| **加字段**        | `PayloadTurnEnded` 加 `CostUS`                   | 兼容。反序列化把老 Event 的该字段读成零值;Fold 端要能处理零值。**新字段语义须文档化,避免"零值被误读为有效值"**。                                                                         |
| **删字段**        | `PayloadTurnEnded` 去掉 `LatencyMS`              | 兼容。反序列化时该字段被丢弃;Fold 端不能再依赖它。**旧代码读新 Event 也不受影响**——JSON 里那个字段还在,只是不 map 到任何 struct field。 |
| **改语义/改类型** | `Budget.MaxTokens` 从 `int` 改成 `map[Model]int` | **不兼容**。须新增一个 `PayloadTaskCreatedV2` + 迁移脚本,不允许原地改。**即使 JSON 形状不变,语义变更也视为 break**。                                                                 |
...
| L4 · 专用 EventStore | EventStoreDB / Kafka(retention topic,非 compacted) | 高吞吐、跨服务共享事件流     |
...
2. **可以只读 `EventStore.Load`**,按 `task_id` 过滤后展开消息原文;**`as_of_seq` 必须与 `State.View` 已折叠的最大 seq 一致**——禁止用更新的 Event 做 Project 而 State 尚未 Apply(见 ADR-002)。
3. **禁止**用 EventStore 绕过 Fold 做生命周期判断,也禁止在 Assemble 里发外部请求。

这条切分让 SessionView 保持小(每个 Session 通常几百字节到数 KB,随 Task/Turn 数量与 WorkingSet 上限线性增长,不是 Event 流的镜像),`View()` 可以频繁调用。ch04 会展开六层 Context 与 WorkingSet 裁剪策略。
...
func UnmarshalEvent(data []byte) (domain.Event, error) {
    ...
    return domain.Event{
        ...
        Type: dto.Type, Payload: derefPayload(payload), TS: parseRFC3339(dto.TS), CausedBy: dto.CausedBy,
    }, nil
}
```
...
| Wall clock 回退            | `ts` 非单调             | 记录 metric,继续(排序不依赖 `ts`,见 §3.3.1;`ts` 不作单调不变量)                         |
