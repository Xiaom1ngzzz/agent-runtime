...
中间做四件事,每一件都是**编译器的经典职责**:

| 阶段 | 名字 | 做什么 |
|---|---|---|
| **1. Layout** | 布局 | 把 six-layer Context 排成 Provider 认识的 message 序列(顺序 + role 归属) |
| **2. Type-check** | 类型检查 | 验证 Messages 合法(role 序列合规、tool_call_id 匹配、schema 完整) |
| **3. Optimize** | 优化 | 合并连续同 role、去重、prompt cache 边界对齐 |
...
**Prompt Compiler 把这四件事集中在一个组件里,业务代码只关心 Context。**

---

## 6.3 六层输入 → Messages 的 Layout(反例先行)
...
**为什么 Memory Refs 靠后**:LLM 在长上下文里对**位置末尾**的注意力更集中(见 §6.9 "Lost in the Middle" 效应)。检索出的相关知识放在**紧邻当前 user turn 之前**,命中率显著高于放在开头。
...
优化不是可选。生产上一个真实数字:一个跑了 200 轮的 Session,朴素 emit 出的 Messages 大概 60K tokens;做完下面这些优化后 20K tokens,成本节省 66%。
...
用户日常问题之一:**"如何让 LLM 稳定输出结构化 JSON"**。这一节展开。
...
### 6.8.2 PromptStore(参考实现)

```go
// runtime-go/prompt/store.go(ch06 Round 2)
...
| Prompt Cache 语法错误 | 静默降级为不 cache,记 metric | `PromptCacheDisabled{reason}` (可选,新 EventType) | 否 |
| Tool schema 无效 | 移除该 tool,记录 | `ToolBindFailed{name, reason}` | 否(LLM 可能用 fallback 回应) |
...
runtime-go/
  prompt/
    prompt.go        (已存在,扩展签名带 Provider)
    reference.go     (新: ReferenceCompiler - 中立格式)
    anthropic.go     (新: AnthropicCompiler)
    openai.go        (新: OpenAICompiler)
    store.go         (新: PromptStore + Template 类型)

runtime-rs/src/
  prompt.rs          (扩展)
  compiler_reference.rs  (新)
  compiler_anthropic.rs  (新)
  compiler_openai.rs     (新)
  prompt_store.rs        (新)
```
...
- Anthropic 版本:system 独立字段,不在 messages 数组;tool schema key 为 `input_schema`
- OpenAI 版本:system 是 messages[0];tool schema key 为 `parameters`
- 两版本的 messages 数量与 role 排布符合各自约束(Anthropic 不允许连续 user;OpenAI 允许)
- Type-check 断言:构造一个 "role=tool 缺 tool_call_id" 的 Context,两个 Compiler 都返回 `PromptError`
...
| 决策 | 选择 | 代价 | 什么情况下会被推翻 |
|---|---|---|---|
| Compile 命名 | 用编译器术语(Layout/Type-check/Optimize/Emit) | 对没接触过编译原理的读者陡一些 | 若面向"prompt 工程师"读者群,可以换成 Compose/Validate/Trim/Render |
...
| Prompt 版本管理 | 独立 PromptStore + 版本号 | 需要额外基础设施 | 极小项目可以放代码常量里,但要有版本 tag |
...
- Prompt Compiler 的四段职责:Layout / Type-check / Optimize / Emit,对齐编译器经典分工。
...
- Optimize 三件事:合并同 role、去重、Cache 边界对齐。生产真实数字:60K → 20K tokens。
...
- 参考实现(Round 2 落地):
  - Go: `runtime-go/prompt/{reference,anthropic,openai,store}.go`
  - Rust: `runtime-rs/src/{compiler_reference,compiler_anthropic,compiler_openai,prompt_store}.rs`
- 相关章节:`ch04-context-engine.md`(§4.8 Compile 引入)、`ch05-memory.md`(§5.5 Memory Refs 的渲染在 Compile 里落地)、`ch07-planner.md`(Task Graph 与 TaskFrame 的关系)
- 研究/工程参考:
