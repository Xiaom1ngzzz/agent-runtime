...
resp, err := client.Chat(ctx, msgs, tools)
if err != nil {
    log.Error("bad prompt", "err", err) // 日志到底哪里错了?
}
_ = resp
```
...
```rust
// ❌ 反例:直接发,让 Provider 报错
match client.chat(&msgs, &tools) {
    Ok(resp) => { /* ... */ }
    Err(e) => log::error!("bad prompt: {e}"), // 日志到底哪里错了?
}
```
...
```go
// ❌ 反例
func compile(ctx domain.Context) []domain.Message {
    msgs := []domain.Message{}
    msgs = append(msgs, instructions()) // 每次读磁盘或配置
    msgs = append(msgs, taskFrame(ctx.Task))
    // ...
    return msgs
}
```
...
```rust
// ❌ 反例
fn compile(ctx: &Context) -> Vec<Message> {
    let mut msgs = Vec::new();
    msgs.push(instructions()); // 每次读磁盘或配置
    msgs.push(task_frame(&ctx.task));
    // ...
    msgs
}
```
