...
```mermaid
%%{init: {"flowchart": {"htmlLabels": true, "wrappingWidth": 720, "rankSpacing": 40, "nodeSpacing": 10}}}%%
flowchart TB
    subgraph Layers["六层输入"]
        ...
        subgraph R1[" "]
            direction RL
            F1["<div style='width:150px;text-align:left'>每 Session 变一次</div>"]
            L1["<div style='width:520px;text-align:left'>1. Instructions (system prompt / role card / policy)</div>"]
            F1 --> L1
        end

        subgraph R2[" "]
            direction RL
            F2["<div style='width:150px;text-align:left'>每 Task 变一次</div>"]
            L2["<div style='width:520px;text-align:left'>2. Task Frame (Task.Goal + Task.Budget + constraints)</div>"]
            F2 --> L2
        end

        subgraph R3[" "]
            direction RL
            F3["<div style='width:150px;text-align:left'>每 Turn 变一次</div>"]
            L3["<div style='width:520px;text-align:left'>3. Working Set (最近 N 个 Turn 的原文消息)</div>"]
            F3 --> L3
        end

        subgraph R4[" "]
            direction RL
            F4["<div style='width:150px;text-align:left'>Compressor 触发</div>"]
            L4["<div style='width:520px;text-align:left'>4. Compressed History (更早 Turns 的结构化摘要)</div>"]
            F4 --> L4
        end

        subgraph R5[" "]
            direction RL
            F5["<div style='width:150px;text-align:left'>按查询触发 (ch05)</div>"]
            L5["<div style='width:520px;text-align:left'>5. Memory Refs (向量库/KV 检索出的相关片段)</div>"]
            F5 --> L5
        end

        subgraph R6[" "]
            direction RL
            F6["<div style='width:150px;text-align:left'>每 Session/Task</div>"]
            L6["<div style='width:520px;text-align:left'>6. Tool Specs (可用工具的 JSON Schema)</div>"]
            F6 --> L6
        end
...
把 Context 的生命周期画成一张图,并给每一档工程语义。原图见 [`diagrams/ch04-context-lifecycle.mmd`](../diagrams/ch04-context-lifecycle.mmd)。
...
flowchart LR
    subgraph New["New"]
        direction TB
        N1["创建中的 Turn"]
        N2["Turn 结束<br/>自动进入 Working Set"]
        N1 --- N2
    end

    subgraph Hot["Hot"]
        direction TB
        H1["Working Set 里的原文 turn"]
        H2["Working Set 满了 (≥ N)<br/>触发 Compressor"]
        H1 --- H2
    end

    subgraph Warm["Warm"]
        direction TB
        W1["Compressed History<br/>里的结构化摘要"]
        W2["可选归档到 Memory<br/>ch05 路径 A"]
        W1 --- W2
    end

    subgraph Cold["Cold"]
        direction TB
        C1["Memory-only 向量<br/>不再直接入 Prompt"]
        C2["按 Retention<br/>策略归档"]
        C1 --- C2
    end

    subgraph Archive["Archive"]
        direction TB
        A1["冷存储<br/>不再进入 Prompt"]
        A2["永不删除<br/>合规 / 审计"]
        A1 --- A2
    end
...
EventStore.Load        (from_seq=X)  →  events
                                        ...
                                        SessionView + WorkingSet + Summaries + MemoryRefs
                                            ↓ Assemble (纯, as_of_seq = view.MaxSeq)
                                        Context
                                            ↓ Compile (纯, §4.8)
                                        Messages
```
...
Assemble(sid, tid) :=
    ...
    events     := EventStore.Load(sid)                            // 只读展开;seq ≤ view.MaxSeq
...
`layout` 是纯的:输入相同则输出相同。Assemble **不写状态、不调用 LLM/Memory、不读取墙钟**;允许的唯一 IO 是确定性地只读 `State.View` 与 `EventStore.Load`,用于展开 WorkingSet 指向的消息原文。**`as_of_seq` 契约**:只读 Event 的 `seq` 不得超过 `view.MaxSeq`(ADR-002)。
...
```rust
// ❌ 反例:在 Project 里"顺手"起个摘要 LLM 调用
fn assemble_wrong(&self, sid: &str, _tid: &str) -> Result<Context, ContextError> {
    let events = self.store.load(sid)?;
    if tokens_of(&events) > 30_000 {
        let summary = call_llm_to_summarize(&events[..80]); // ⚠️ Project 里发外部请求
        // ...
    }
    // ...
}
...
Assemble 里那句 `selectRelevantSummaries(view.Summaries, working)` 看起来很朴素,但**"什么算相关"是纯逻辑**。参考实现用 WorkingSet 的最小 seq 去掉与原文重叠的 Summary;需要多 Task 隔离的实现还应增加 `summary.task_id == tid` 过滤。**Summaries 与 WorkingSet 的遍历顺序须确定**(如按 `FromSeq` 升序),否则同一份 View 可能拼出不同 Context。**不允许**:
...
> **关于"Round 2"标注**:本书参考实现按迭代阶段推进,代码注释与"实现状态"提示里出现的"Round 2"指当前已随书落地的最小闭环实现;凡标注为"Round 2 未包含/留作扩展"的内容,均是该阶段之外的生产化工作,不代表设计缺失。后续章节沿用同一约定,不再逐处解释。
...
```rust
// runtime-rs/src/compressor/mod.rs(ch04 Round 2 落地)
...
loop {
    // 正常 Turn
    rt.step(sid, tid, turn_id)?;

    // 每 Turn 结束尝试压缩一次
    if let Ok(events) = compressor.tick(sid) {
        if !events.is_empty() {
            store.append(&mut events.clone())?;
            state.apply(&events)?;
        }
    }
}
...
**基线策略**:按 Turn 数(参考实现 `compressor.ByTurns`)。按 Task 边界的实现与之同构——把判定从"WorkingSet 长度"换成"`TaskEnded` 到来",参考实现未单独包含。按 Token 需要 tokenizer(每个 Provider 都不一样),ch08 落地版也未引入该依赖,留作扩展。
...
```rust
// ❌ 反例
fn assemble_wrong2(&self) -> Result<Context, ContextError> {
    let mut ctx = self.build_context()?;
    if tokens_of(&ctx) > WINDOW_LIMIT {
        ctx = compress_on_the_fly(ctx)?; // 触发 LLM 摘要
    }
    Ok(ctx)
}
```
...
```rust
// runtime-rs/src/domain/summary.rs(ch04 Round 2 落地)
pub struct Summary {
...
**为什么带 `AtSeq`**:决策/事实可沿 `AtSeq` 定位到 EventStore 中的原文 Event,供人工核查或按需重放。**摘要本身不是原文的完整替身**——压缩必然丢细节,`AtSeq` 提供"回到原文"的钩子,不保证自动还原全部上下文。
...
prompt := fmt.Sprintf("Summarize these %d turns: ...", n)
```
...
```rust
// 每次 Compressor 触发,把整段 turns 从头喂给 LLM
let prompt = format!("Summarize these {n} turns: ...");
```
...
规则:
1. tool_results 只保留**有 side effect 或下游会复用**的返回值;纯 read-only probe 可省略
2. tool_results 的 key 用 `"tool_name:stable_arg_hash"`,值保留**原文关键字段**,禁止意译
3. decisions_made 必须带 at_seq
4. 有已存在的 Summary,新事实要 merge 到里面(见 <prior_summary>)
5. 用原文关键词,不改写
...
```rust
// runtime-rs/src/domain/summary.rs
pub struct Progress {
    pub goal: String,
    pub done: Vec<Step>,
    pub next: Vec<Step>,
...
跟 Summary 一样是"上游组件产出 Event、Assemble 只读"——但生成方不是 Compressor,而是 **ch07 的 Planner**(它掌握 Task 结构,知道"做到哪了")。设计上的触发时机:
...
**Progress 更新是幂等的**——同一版本号写两次结果一样。这条纪律让生成方可以放心重跑(比如 Loop 崩了,恢复后又触发一次)。

> **实现状态**:Round 2 已落地 Progress schema、`ProgressUpdated` Fold 和 `<task_progress>` Prompt 渲染;生成逻辑在 ch07 落地(`planner.BuildProgressFromView`,Plan 时机触发)。按 user_input / error 即时刷新留扩展。

---

## 4.8 Prompt 编译器:从 Context 到 Messages
...
```rust
// runtime-rs/src/prompt/mod.rs(ch06 Round 2 落地)
pub struct ReferenceCompiler;
pub struct AnthropicCompiler { pub model: String }
pub struct OpenAICompiler { pub model: String }

// 三者都实现 PromptCompiler trait;换 Provider = 换实例:
let rt = Runtime {
    prompt: Arc::new(AnthropicCompiler { model: "claude-opus-4-7".into() }),
    // ...
};
```
...
| Tool Specs 拉取失败 | 降级到空 tools 列表——LLM 只能自然语言应答 | `ToolBindFailed{reason}`(ch08 随工具注册表落地) | 否 |
...
| 压缩策略数 | 基线一种:按 turn 数(`ByTurns`);Task 边界策略同构,未单独落地 | 按 token 需要 tokenizer 依赖,先不做 | 引入 tokenizer 依赖后启用"按 token"策略(ch08 落地版未包含) |
...
下一章 **第 5 章 · 记忆架构** 会展开 Memory Refs 这一层:向量库、KV、跨 Session 长期记忆的接入协议。
