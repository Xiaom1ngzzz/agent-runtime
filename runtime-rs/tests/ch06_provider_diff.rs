...
fn ch06_provider_diff_anthropic() {
    ...
    assert!(
        !req.tools[0].input_schema.is_empty(),
        "input_schema should be set"
    );
    ...
    let found_tool_result = req.messages.iter().any(|m| {
        m.content
            .iter()
            .any(|c| c.kind == "tool_result" && c.tool_use_id == "call_1")
    });
    assert!(
        found_tool_result,
        "tool_result should appear as content array item"
    );
    ...
    let found_tool_use = req.messages.iter().any(|m| {
        m.role == "assistant"
            && m.content
                .iter()
                .any(|c| c.kind == "tool_use" && c.tool_use_id == "call_1")
    });
    assert!(
        found_tool_use,
        "tool_use should appear as content array item"
    );
}
...
fn ch06_provider_diff_openai() {
    ...
    assert!(
        sys_count >= 2,
        "expected 2+ system messages, got {}",
        sys_count
    );
    ...
    assert_eq!(req.tools.len(), 1);
    assert!(
        !req.tools[0].function_parameters.is_empty(),
        "function_parameters should be set"
    );
    assert_eq!(req.tools[0].kind, "function");

    // 3. assistant.tool_calls 是独立字段
    let found_tool_call = req
        .messages
        .iter()
        .any(|m| m.role == "assistant" && m.tool_calls.iter().any(|tc| tc.id == "call_1"));
    assert!(
        found_tool_call,
        "expected tool_call id=call_1 in assistant.tool_calls"
    );

    // 4. tool response 是独立 role=tool
    let found_tool_response = req
        .messages
        .iter()
        .any(|m| m.role == "tool" && m.tool_call_id == "call_1");
    assert!(
        found_tool_response,
        "expected role=tool with tool_call_id=call_1"
    );
}
...
fn ch06_type_check_catches_tool_call_id_mismatch() {
    ...
    assert!(
        ...
        "error should mention tool_call_id: {}",
        msg
    );
}

#[test]
fn ch06_anthropic_merges_consecutive_user() {
    let ctx = Context {
        messages: vec![
            Message {
                role: "user".into(),
    ...
    let req = c
        .compile_to_provider(&ctx)
        .expect("anthropic merges consecutive user");
    assert_eq!(req.messages.len(), 1);
    assert_eq!(req.messages[0].role, "user");
    assert_eq!(req.messages[0].content.len(), 2);

    let oc = OpenAICompiler {
        model: "gpt-5".into(),
    };
