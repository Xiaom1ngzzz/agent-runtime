...
fn ch05_memory_cycle() {
    ...
    mem.upsert(MemoryItem {
        id: "m1".into(), source: "user_pref".into(), kind: MemoryKind::Semantic,
        key: "user:42:diet".into(), content: "不吃辣,偏好清淡".into(),
        embedding: embed_text("不吃辣", EMBED_DIM),
        tags: vec!["user:42".into()], version: 1,
        ..Default::default()
    }).unwrap();
    mem.upsert(MemoryItem {
        id: "m2".into(), source: "user_pref".into(), kind: MemoryKind::Semantic,
        key: "user:42:email".into(), content: "xiaoming@example.com".into(),
        embedding: embed_text("邮箱 email", EMBED_DIM),
        tags: vec!["user:42".into()], version: 1,
        ..Default::default()
    }).unwrap();
    mem.upsert(MemoryItem {
        id: "m3".into(), source: "kb.docs".into(), kind: MemoryKind::Semantic,
        key: "kb:travel:policy".into(), content: "差旅政策".into(),
        embedding: embed_text("差旅政策 travel policy", EMBED_DIM),
        tags: vec!["domain:travel".into()], version: 1,
        ..Default::default()
    }).unwrap();
    mem.upsert(MemoryItem {
        id: "m4".into(), source: "session_summary".into(), kind: MemoryKind::Episodic,
        ...
        origin_session: "s0".into(), origin_task_id: "t0".into(),
        origin_seq_from: 1, origin_seq_to: 40,
        version: 1,
        ..Default::default()
    }).unwrap();
    ...
    // 断言 B: score 降序
    for i in 1..refs.len() {
        assert!(refs[i].score <= refs[i - 1].score,
    ...
    let mut seed: Vec<Event> = vec![
        Event {
            id: String::new(), session_id: sid.into(), task_id: String::new(),
            turn_id: String::new(), ts: None, caused_by: String::new(), seq: 0,
        ...
        Event {
            id: String::new(), session_id: sid.into(), task_id: String::new(),
            turn_id: String::new(), ts: None, caused_by: String::new(), seq: 0,
            payload: EventPayload::UserSpoke(PayloadUserSpoke { text: "帮我订机票".into() }),
        },
        Event {
            id: String::new(), session_id: sid.into(), task_id: tid.into(),
            turn_id: String::new(), ts: None, caused_by: String::new(), seq: 0,
            payload: EventPayload::TaskCreated(PayloadTaskCreated {
                goal: "帮我订机票".into(), ..Default::default()
            }),
        },
        Event {
            id: String::new(), session_id: sid.into(), task_id: tid.into(),
            turn_id: turn.into(), ts: None, caused_by: String::new(), seq: 0,
    ...
    let has_memory = ctx.messages.iter().any(|m| m.content.contains("<memory_ref"));
...
fn ch05_min_score_filter() {
    let mem = InMemStore::new();
    mem.upsert(MemoryItem {
        id: "a".into(), key: "a".into(), content: "苹果 apple".into(),
        embedding: embed_text("苹果 apple", EMBED_DIM),
        kind: MemoryKind::Semantic, version: 1,
        ..Default::default()
    }).unwrap();
    mem.upsert(MemoryItem {
        id: "b".into(), key: "b".into(), content: "订机票 book flight".into(),
    ...
    let refs = mem.query(&Query {
        semantic: "订机票".into(), top_k: 10, min_score: 0.9,
        ..Default::default()
    }).unwrap();
