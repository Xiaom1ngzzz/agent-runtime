...
| **Project** | `SessionView → Context`（可只读展开 EventStore 中的消息原文;`as_of_seq` 须与 View 一致,见下） | `ContextEngine.Assemble`     | 纯 |
...
2. **Project 以 SessionView 为主输入,不写 Event、不发外部请求**。生命周期判断(哪个 Task/Turn 在跑)只能来自 Fold 后的 SessionView,不能绕过 State 直读 EventStore。若需要 LLM 消息原文,可对 EventStore 做**只读**加载,但 **`as_of_seq` 不得超过 `SessionView` 已折叠的最大 seq**——禁止在 State 尚未 Apply 的新 Event 上投影。按 SessionView.WorkingSet 等字段展开已提交的事实——不是"第二套状态源"。ContextEngine 若需要"新鲜数据",必须让 LLM 通过 ToolCall 触发,不能自作主张。
...
- **只承担本次 Turn 的结束事件**:SessionOpened、TaskCreated/Ended、TurnStarted 由调用方(上层 Loop)追加;参考协调器在 Step 成功末尾追加 `TurnEnded`。
...
**核心哲学**:**不承诺原子性,尽力保证可审计**。当前一次性 Executor 在外部副作用成功、结果 Event 落库前仍有崩溃窗口;生产实现需用幂等 CallID、outbox 或 submit/resume 收窄该窗口。已落库的事实可供下一个 Turn 或 Human-in-the-loop 决定补救。
...
- **观测点自然对齐**:5 段转换可作为 5 个 Span 边界;ch10 当前只提供 Eval 指标,OTel Attribute 映射仍需另行版本化。
- **审计有据**:已成功落库的 Emit 事件可沿 `CausedBy` 链回溯;崩溃窗口内的外部副作用需依赖 outbox/幂等协议补齐。
