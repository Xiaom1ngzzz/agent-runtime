...
| **Project** | `SessionView → Context`               | `ContextEngine.Assemble`     | 纯 |
...
2. **Project 只读 State,不写 Event、不发外部请求**。ContextEngine 若需要"新鲜数据",必须让 LLM 通过 ToolCall 触发,不能自作主张。
...
- **每追加一条 Event 都同时 Apply 到 State**——`EventStore.Append` 与 `State.Apply` 在同一临界区完成。
...
- **每段转换必须是可观测的一个 Span**(见 [ch02 §2.6](../chapters/ch02-runtime-dataflow.md#26-观测点每根箭头一个-span))。
...
- **观测点自然对齐**:5 段转换 = 5 个 Span 边界,与 ch10 Observability 规范一一对应,不用事后打补丁。
